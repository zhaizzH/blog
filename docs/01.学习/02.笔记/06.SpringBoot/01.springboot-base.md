...
permalink: /study/springboot-base
categories:
  - 学习
tags:
  - springboot
...
![image-20210911132335452](https://img.zhaizz.top/file/blog-website/springboot/image-20210911132335452.png)

2. **编写 `web3.0` 的配置类**

   作为 web 程序，`web3.0` 的配置类不能缺少，而这个配置类还是比较麻烦的，代码如下

![image-20210911133112602](https://img.zhaizz.top/file/blog-website/springboot/image-20210911133112602.png)

3. **编写 SpringMVC 的配置类**

![image-20210911133219847](https://img.zhaizz.top/file/blog-website/springboot/image-20210911133219847.png)

做到这只是将工程的架子搭起来。要想被外界访问，最起码还需要提供一个 Controller 类，在该类中提供一个方法。

4. **编写 Controller 类**

![image-20210911133532151](https://img.zhaizz.top/file/blog-website/springboot/image-20210911133532151.png)
...
![image-20210911155135008](https://img.zhaizz.top/file/blog-website/springboot/image-20210911155135008.png)

- 选择 `Spring Initializr`，用来创建 SpringBoot 工程

  以前我们选择的是 `Maven`，今天选择 `Spring Initializr` 来快速构建 SpringBoot 工程。而在 `Module SDK` 这一项选择我们安装的 JDK 版本。

![image-20210911155249493](https://img.zhaizz.top/file/blog-website/springboot/image-20210911155249493.png)

- 对 SpringBoot 工程进行相关的设置

  我们使用这种方式构建的 SpringBoot 工程其实也是 Maven 工程，而该方式只是一种快速构建的方式而已。

![image-20210911155916899](https://img.zhaizz.top/file/blog-website/springboot/image-20210911155916899.png)
...
![image-20210911160040328](https://img.zhaizz.top/file/blog-website/springboot/image-20210911160040328.png)

- 下图界面不需要任何修改，直接点击 `Finish` 完成 SpringBoot 工程的构建

![image-20210911160353534](https://img.zhaizz.top/file/blog-website/springboot/image-20210911160353534.png)

经过以上步骤后就创建了如下结构的模块，它会帮我们自动生成一个 Application 类，而该类一会再启动服务器时会用到

![image-20210911160541833](https://img.zhaizz.top/file/blog-website/springboot/image-20210911160541833.png)
...
![image-20210911165642280](https://img.zhaizz.top/file/blog-website/springboot/image-20210911165642280.png)

#### 进行测试

使用 Postman 工具来测试我们的程序

![image-20210911160850121](https://img.zhaizz.top/file/blog-website/springboot/image-20210911160850121.png)
...
![image-20210911172200292](https://img.zhaizz.top/file/blog-website/springboot/image-20210911172200292.png)
...
![image-20210911173712014](https://img.zhaizz.top/file/blog-website/springboot/image-20210911173712014.png)

然后点击 `Spring Initializr` 超链接就会跳转到如下页面

![image-20210911174110687](https://img.zhaizz.top/file/blog-website/springboot/image-20210911174110687.png)
...
![image-20210911174650679](https://img.zhaizz.top/file/blog-website/springboot/image-20210911174650679.png)

#### 生成工程

以上步骤完成后就可以生成 SpringBoot 工程了。在页面的最下方点击 `GENERATE CTRL + 回车` 按钮生成工程并下载到本地，如下图所示

![image-20210911175222857](https://img.zhaizz.top/file/blog-website/springboot/image-20210911175222857.png)

打开下载好的压缩包可以看到工程结构和使用 Idea 生成的一模一样，如下图

![image-20210911175502834](https://img.zhaizz.top/file/blog-website/springboot/image-20210911175502834.png)
...
![image-20210911180828611](https://img.zhaizz.top/file/blog-website/springboot/image-20210911180828611.png)
...
![image-20210911181714437](https://img.zhaizz.top/file/blog-website/springboot/image-20210911181714437.png)
...
![image-20210911182956629](https://img.zhaizz.top/file/blog-website/springboot/image-20210911182956629.png)
...
![image-20210918220338109](https://img.zhaizz.top/file/blog-website/springboot/image-20210918220338109.png)
...
![image-20210918220855024](https://img.zhaizz.top/file/blog-website/springboot/image-20210918220855024.png)

再进入到该父工程中，在该工程中我们可以看到配置内容结构如下图所示

![image-20210918221042947](https://img.zhaizz.top/file/blog-website/springboot/image-20210918221042947.png)

上图中的 `properties` 标签中定义了各个技术软件依赖的版本，避免了我们在使用不同软件技术时考虑版本的兼容问题。在 `properties` 中我们找 `servlet` 和 `mysql` 的版本如下图

![image-20210918221511249](https://img.zhaizz.top/file/blog-website/springboot/image-20210918221511249.png)
...
![image-20210918221942453](https://img.zhaizz.top/file/blog-website/springboot/image-20210918221942453.png)

看完了父工程中 `pom.xml` 的配置后不难理解我们工程的依赖为什么都没有配置 `version`。

#### 探索依赖

在我们创建的工程中的 `pom.xml` 中配置了如下依赖

![image-20210918222321402](https://img.zhaizz.top/file/blog-website/springboot/image-20210918222321402.png)

进入到该依赖，查看 `pom.xml` 的依赖会发现它引入了如下的依赖

![image-20210918222607469](https://img.zhaizz.top/file/blog-website/springboot/image-20210918222607469.png)
...
![image-20210918232512707](https://img.zhaizz.top/file/blog-website/springboot/image-20210918232512707.png)
...
![image-20210918232904623](https://img.zhaizz.top/file/blog-website/springboot/image-20210918232904623.png)
